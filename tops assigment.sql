...
select concat(first_name,last_name) as 'ENAME',department_name as 'deptno',salary,commission_pct from employees join departments
on employees.employee_id = departments.department_id
...
--	  the length of the names, for all employees whose name starts with J, A, or M. Give each column an appropriate label. Sort the results by the employees last names.
	select concat(upper(substr(last_name,1,1)),substr(last_name,2)) as last_name,char_length(last_name) as name_length from employees
